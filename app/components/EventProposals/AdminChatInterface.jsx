...
const AdminChatInterface = () => {
    ...
    const { doc_ref, type } = route.params;
        ...
        const updateLastReadMsg = async () => {
            try {
                await runTransaction(FIRESTORE_DB, async (transaction) => {
                    ...
                    const seekerDoc = await transaction.get(seekerDocRef);
    ...
    return (
        <View style={style.container}>
            <Header name={seekerName} doc_ref={doc_ref} navigator={navigator} type={type}/>
            <FlatList 
            ...
            />
            <Footer message={inputMessage} setMessage={setInputMessage} doc_ref={doc_ref} type={type}/>
        </View>
    );
}
...
const Header = ({ name, doc_ref, navigator, type }) => {
    return (
        <View style={style.header}>
            ...
            <TouchableOpacity
                onPress={() => deleteDatabase(doc_ref, navigator, type)}>
                <Image
                    source={delete_icon}
                />
            </TouchableOpacity>
...
const Footer = ({ message, setMessage, doc_ref, type }) => {
    return (
        <View style={style.footer}>
            ...
            />
            <TouchableOpacity
                onPress={() => addMessageToDatabase(message, setMessage, doc_ref, type)}>
                <Image
                    source={send_icon}
                />
            </TouchableOpacity>
...
const deleteDatabase = async (doc_ref, navigator, type) => {
    const collectionRef = collection(FIRESTORE_DB, `${type}ChatRoom`);
...
const addMessageToDatabase = async (message, setMessage, doc_ref, type) => {
    if (message == "") {
        return;
    }
    const NOW = new Date();
